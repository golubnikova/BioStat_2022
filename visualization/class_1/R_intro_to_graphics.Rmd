...
У кого-то из вас мог получиться другой результат. Расскажите где это получилось и почему так могло произойти?
```
...
1. Неправильный формат данных поданных на аргумент функции
...
ggplot(data = pima, 
       aes(x = glucose)) +
  geom_density() +
  geom_vline(aes(xintercept = glucose_mean)) + #Вертикальная линия
  annotate("text", 
           x= glucose_mean+30, 
           y=0.013, 
...
```{r}
ggplot() +
  geom_histogram(data = pima, 
               aes(x = pregnant), bins = 30) +
  theme_minimal() +
  ggtitle('Распределения числа беременностей') + 
  labs(x = 'Беременность', y = 'Количество') +
  theme( # Ниже идут настройки тем (их огромное количество, но мы меняем лишь две)
    title = element_text(size = 12), # Задаем стандартный размер текста для любого элемента графика
    axis.title.y = element_text(size=14) # Задаём размер для названия оси y отдельно + 
    )
...
ggplot() +
  geom_histogram(data = pima, 
               aes(x = insulin, fill = age_group), 
               alpha = 0.5 # Показатель альфа указывает насколько прозрачным должно быть отображение фигур. Измеряется от 0 до 1.
               ) +
  theme_minimal()

ggplot() +
  geom_histogram(data = pima[pima$insulin != 0,], 
               aes(x = insulin, fill = age_group), alpha = 0.5) +
  theme_minimal()
...
```{r}
pima$was_pregnant <- ifelse(pima$pregnant == 0, 'no', 'yes')
ggplot() +
  geom_bar(data = pima, 
           aes(x = diabetes, fill = was_pregnant), 
           position="dodge") +
  theme_minimal()
...
pima %>% 
  ...
  ggplot() + 
    geom_point(size=3,
               aes(x=mass, y=triceps, color = pressure))
```
...
```{r}
pima %>% 
  filter(mass != 0 & triceps != 0 & pressure != 0 & insulin != 0) %>% 
  ggplot() + 
    geom_point(aes(x=mass, y=triceps, color = pressure, shape = age_group, alpha = insulin, size = pedigree), size=3)

pima %>% 
  filter(mass != 0 & triceps != 0 & pressure != 0 & insulin != 0) %>% 
  ggplot() + 
    geom_point(aes(x=mass, y=triceps, color = pressure, shape = age_group, alpha = diabetes, size=insulin), size=3)
...
scatter <- pima %>% 
  ...
  ggplot(aes(x=insulin, y=glucose, color = age_group, fill = age_group, group = age_group)) + 
  geom_point() +
...
combine_plot <- ggarrange(scatter, density_plot,
                          ncol = 2, nrow = 1)
...
# P.S. Специфические типы графиков (дополнительная темы)
