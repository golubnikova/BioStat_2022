...
# Binomial Test
#Позволяет ответить на вопрос: насколько фактическое отличается от теоретически предсказанного
binom.test(x = 5, n = 20, p = 0.5)
binom.test(t1)


# Chi-Square
#Проверяет, насколько эмпирическое распределение отличается от случайного (проверяет то же, что биномиальный тест)
t1
chisq.test(t1)
